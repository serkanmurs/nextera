...
const PD: Record<string,{t:string;c:string;bg:string;e:string;s:string[];w:string[];summary:string}> = {
...
function RegisterScreen({ onRegister, onGoLogin }: { onRegister: (form: any) => void; onGoLogin: () => void }) {
  const [form, setForm] = useState<any>({ name: "", email: "", password: "", gender: "", phone: "", city: "", bio: "", kvkk: false });
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  const update = (key: string, val: any) => { setForm((p: any) => ({ ...p, [key]: val })); setError(""); };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #F97316 0%, #FB923C 30%, #F8FAFC 100%)", display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 24px" }}>
      ...
      <div style={{ background: "#fff", borderRadius: 24, padding: 28, width: "100%", maxWidth: 380, boxShadow: "0 20px 60px rgba(0,0,0,0.12)" }}>
        ...
        <button onClick={handleRegister} disabled={loading} style={{ background: "#F97316", color: "#fff", border: "none", borderRadius: 12, padding: "12px 24px", fontSize: 15, fontWeight: 700, cursor: "pointer", width: "100%", boxShadow: "0 4px 14px rgba(249,115,22,0.3)", opacity: loading ? 0.7 : 1 }}>
              ...
              {clients.map(c => {
                ...
                return (
                  <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: "1px solid #E2E8F0" }}>
                    <div style={{ width: 44, height: 44, borderRadius: "50%", background: "#0E7490", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 15 }}>{getInitials(c.name)}</div>
                    <div style={{ flex: 1 }}>
        ...
        {upcoming.length === 0 ? <p style={{ color: COLORS.textLight, fontSize: 14, textAlign: "center", padding: 20 }}>Yaklaşan seans yok</p> : upcoming.slice(0, 5).map(s => {
          ...
          return (
            <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: `1px solid ${COLORS.border}` }}>
              <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#0E7490", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 13 }}>{client ? getInitials(client.name) : "?"}</div>
        ...
        {clients.map(c => {
          ...
          return (
            <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: `1px solid ${COLORS.border}` }}>
              <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#0E7490", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 14 }}>{getInitials(c.name)}</div>
          ...
          return (
            <div key={s.id} style={{ padding: "12px 0", borderBottom: `1px solid ${COLORS.border}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#0E7490", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 10 }}>{client ? getInitials(client.name) : "?"}</div>
...
// ==================== CLIENT HOME ====================
function ClientHome({ user, sessions, users, onRate, tasks, journal, onAddTask, onToggleTask, onAddJournal, onEditTask, onDeleteTask, onEditJournal, onDeleteJournal, personalityTest, onSaveTest }: { user: UserType; sessions: SessionType[]; users: UserType[]; onRate: (id: string) => void; tasks: TaskType[]; journal: JournalType[]; onAddTask: (title: string) => void; onToggleTask: (id: string, done: boolean) => void; onAddJournal: (note: string, cat: string) => void; onEditTask: (id: string, title: string) => void; onDeleteTask: (id: string) => void; onEditJournal: (id: string, note: string) => void; onDeleteJournal: (id: string) => void; personalityTest: PersonalityTestType | null; onSaveTest: (r: any) => void }) {
  ...
  return (
    <div style={{ padding: 16 }}>
        ...
        <div style={{ ...cardStyle, background: "linear-gradient(135deg, #0E7490, #155E75)", color: "#fff", border: "none" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, opacity: 0.9 }}>🏅 ICF Akredite Profesyonel Danışman</div>
      ...
      {/* Aksiyon Planım */}
      <div style={cardStyle}>
        <h3 style={{ margin: "0 0 14px", fontSize: 16, fontWeight: 800, color: COLORS.text }}>✅ Adımlarım</h3>
        {tasks.length === 0 ? (
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
          <input id="newTaskInput" placeholder="Yeni adım ekle..." style={{ flex: 1, padding: "10px 14px", borderRadius: 10, border: "1.5px solid #E2E8F0", fontSize: 13, outline: "none" }} onKeyDown={(e: any) => { if (e.key === "Enter" && e.target.value.trim()) { onAddTask(e.target.value.trim()); e.target.value = ""; } }} />
          <button onClick={() => { const inp = document.getElementById("newTaskInput") as HTMLInputElement; if (inp?.value.trim()) { onAddTask(inp.value.trim()); inp.value = ""; } }} style={{ padding: "10px 16px", borderRadius: 10, background: COLORS.primary, color: "#fff", border: "none", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Ekle</button>
        </div>
      </div>
...
// ==================== SESSIONS PAGE ====================
function SessionsPage({ user, sessions, users, onBook, onCancel, onDelete, isAdmin }: { user: UserType; sessions: SessionType[]; users: UserType[]; onBook: (d: string, t: string) => void; onCancel: (id: string) => void; onDelete: (id: string) => void; isAdmin: boolean }) {
    ...
    <div style={{ padding: 16 }}>
      ...
      {showBooking && !isAdmin && (
        <div style={cardStyle}>
          ...
          {selectedTime && (
            <button onClick={handleBook} style={{ background: "#0E7490", color: "#fff", border: "none", borderRadius: 12, padding: "12px 24px", fontSize: 15, fontWeight: 700, cursor: "pointer", width: "100%", boxShadow: "none" }}>
      ...
      <div style={cardStyle}>
        ...
        {mySessions.length === 0 ? <p style={{ color: COLORS.textLight, fontSize: 14, textAlign: "center", padding: 20 }}>Yolculuk başlıyor. İlk seansın dönüşümünü burada göreceksin. 🌱</p> : [...mySessions].sort((a, b) => b.date.localeCompare(a.date)).map(s => {
          const client = users.find(u => u.id === s.client_id);
          ...
          return (
            <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: `1px solid ${COLORS.border}` }}>
              {isAdmin && <div style={{ width: 34, height: 34, borderRadius: "50%", background: "#0E7490", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 12 }}>{client ? getInitials(client.name) : "?"}</div>}
              <div style={{ flex: 1 }}>
      ...
      <div style={cardStyle}>
        ...
        {loadingCal ? (
          <p style={{ color: "#64748B", fontSize: 14, textAlign: "center", padding: 16 }}>Yükleniyor...</p>
          ...
          <p style={{ color: "#64748B", fontSize: 13, textAlign: "center", padding: 16 }}>Yaklaşan etkinlik yok.</p>
        ) : (
          <div style={{ textAlign: "center", padding: 16 }}>
            <p style={{ color: "#64748B", fontSize: 13, margin: "0 0 12px" }}>Google Takvim bağlayarak seansları otomatik ekleyin.</p>
            <button onClick={() => window.location.href = "/api/auth/google"} style={{ background: "#0E7490", color: "#fff", border: "none", borderRadius: 12, padding: "10px 20px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>🔗 Google Takvim Bağla</button>
          </div>
        )}
      </div>
      )}
        ...
        {chatPartners.length === 0 ? <p style={{ color: COLORS.textLight, textAlign: "center", padding: 40 }}>Henüz mesaj yok</p> : chatPartners.map(partner => {
          ...
          return (
            <div key={partner.id} onClick={() => setActiveChat(partner.id)}
              ...
              <div style={{ width: 48, height: 48, borderRadius: "50%", background: "#0E7490", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 17, flexShrink: 0 }}>{getInitials(partner.name)}</div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 130px)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "#fff", borderBottom: `1px solid ${COLORS.border}` }}>
        ...
        <div style={{ width: 38, height: 38, borderRadius: "50%", background: "#0E7490", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 14 }}>{partner ? getInitials(partner.name) : "?"}</div>
      ...
      <div style={{ ...cardStyle, textAlign: "center", padding: 28, background: "linear-gradient(135deg, #F0FDFA 0%, #EFF6FF 100%)" }}>
        <div style={{ position: "relative", display: "inline-block" }}>
          {user.avatar && user.avatar.startsWith("http") ? (
            ...
            <div style={{ width: 80, height: 80, borderRadius: "50%", background: "#0E7490", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 28, margin: "0 auto 14px" }}>{getInitials(user.name)}</div>
          )}
          {editing && (
...
export default function NextERAApp() {
  ...
  const fetchData = useCallback(async (userId: string) => {
    try {
      const [usersRes, sessionsRes, messagesRes, notifsRes, tasksRes, journalRes] = await Promise.all([
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0E7490" }}>
  ...
  return (
    <>
      ...
      {/* Header */}
      <div style={{ background: "#0E7490", padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50 }}>
        <div>
          ...
          <div style={{ color: "rgba(255,255,255,0.8)", fontSize: 12, fontWeight: 500 }}>{isAdmin ? "Yönetim Paneli" : "Danışmanlık Platformu"}</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {currentUser?.avatar?.startsWith("http") ? (
              ...
              <div style={{ width: 34, height: 34, borderRadius: "50%", background: "#0E7490", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 12, border: "2px solid rgba(255,255,255,0.4)" }}>{currentUser ? getInitials(currentUser.name) : "?"}</div>
            )}
        </div>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-around", background: "#fff", borderTop: `1px solid ${COLORS.border}`, padding: "8px 0 12px", position: "sticky", bottom: 0, zIndex: 50 }}>
        {navItems.map(({ id, label, Icon, badge }: any) => (
          <div key={id} onClick={() => setPage(id)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, color: page === id ? COLORS.primary : COLORS.textLight, cursor: "pointer", fontSize: 11, fontWeight: page === id ? 700 : 500, padding: "6px 16px", borderRadius: 12, position: "relative" as const, minWidth: 56, minHeight: 44 }}>
